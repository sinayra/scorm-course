...
var APPID = "S1LBOP40O4";
var CLOUDURL = "http://cloud.scorm.com/rs/stat/sd";
var blnDebug = true;
...
function SCORM2004_GetInteractionTimestamp(strInteractionID) {
  ...
  var intInteractionIndex = SCORM2004_FindInteractionIndexFromID(
    strInteractionID
  );
  WriteToDebug("intInteractionIndex=" + intInteractionIndex);
  SCORM2004_ClearErrorInfo();
...
function AICC_SetObjectiveScore(
  ...
  if (intObjIndex != null) {
    ...
    AICC_aryObjectivesRead[parseInt(intNextID, 10)][
      AICC_OBJ_ARRAY_ID
    ] = strObjectiveID;
    AICC_aryObjectivesRead[parseInt(intNextID, 10)][
      AICC_OBJ_ARRAY_SCORE
    ] = intScore;
  ...
  if (intObjIndex != null) {
    ...
    AICC_aryObjectivesWrite[parseInt(intNextID, 10)][
      AICC_OBJ_ARRAY_ID
    ] = strObjectiveID;
    AICC_aryObjectivesWrite[parseInt(intNextID, 10)][
      AICC_OBJ_ARRAY_SCORE
...
function AICC_TranslateTimeToMilliseconds(strCMITime) {
  ...
  if (IsValidCMITimeSpan(strCMITime)) {
    AICC_intPreviouslyAccumulatedMilliseconds = ConvertCMITimeSpanToMS(
      strCMITime
    );
  } else {
    WriteToDebug("ERROR - Invalid CMITimeSpan");
...
function AICC_FormatObjectives(strObjectivesFromLMS) {
  ...
  for (i = 0; i < aryLines.length; i++) {
    ...
    if (strLineType == "j_id") {
      ...
      AICC_aryObjectivesRead[parseInt(strIndex, 10)][
        AICC_OBJ_ARRAY_ID
      ] = strLineValue;
  ...
  LRS.prototype = {
          ...
          if (env.hasCORS) {
            if (env.useXDR && schemeMatches) {
              ...
              if (cfg.allowFail) {
                if (this.alertOnRequestFailure) {
                  alert(
                    "[warning] LRS invalid: cross domain request for differing scheme in IE with XDR"
                  );
                }
              } else {
                if (this.alertOnRequestFailure) {
            ...
            if (cfg.allowFail) {
              if (this.alertOnRequestFailure) {
                alert(
                  "[warning] LRS invalid: cross domain requests not supported in this browser"
                );
              }
            } else {
              if (this.alertOnRequestFailure) {
    ...
    sendRequest: function (cfg) {
      ...
      function requestComplete() {
        self.log(
          "requestComplete: " + finished + ", xhr.status: " + xhr.status
        );
      ...
      if (this._requestMode === NATIVE) {
        ...
        this.log(
          "sendRequest using XMLHttpRequest - async: " +
            (typeof cfg.callback !== "undefined")
        );
...
(function () {
  ...
  ContextActivities.prototype = {
    ...
    init: function (cfg) {
      ...
      var i,
        j,
        objProps = ["parent", "grouping", "other"],
        prop,
        val;
      cfg = cfg || {};
...
function LMSStandardAPI(strStandard) {
  ...
  eval(
    "this.DisplayMessageOnTimeout = " + strStandard + "_DisplayMessageOnTimeout"
  );
  ...
  eval(
    "this.RecordFillInInteraction = " + strStandard + "_RecordFillInInteraction"
  );
  eval(
  ...
  eval(
    "this.RecordLikertInteraction = " + strStandard + "_RecordLikertInteraction"
  );
  eval(
  ...
  eval(
    "this.SetObjectiveDescription = " + strStandard + "_SetObjectiveDescription"
  );
  eval(
    "this.GetObjectiveDescription = " + strStandard + "_GetObjectiveDescription"
  );
  ...
  eval(
    "this.GetInteractionTimestamp = " + strStandard + "_GetInteractionTimestamp"
  );
  eval(
  ...
  eval(
    "this.GetInteractionWeighting = " + strStandard + "_GetInteractionWeighting"
  );
  eval(
